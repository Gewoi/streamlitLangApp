    ...
    for step in lesson.get('steps', []):
        ...
        if step_type == 'markdown':
            ...
            if step.get('solution_display'):
                ...
                for line in step['solution_display'].split('\n'):
                    lines.append(f'      {line}')
            if step.get('images'):
                lines.append('    images:')
                for img in step['images']:
...
class StepCard(ttk.Frame):
    ...
    def __init__(self, parent, step_data: Dict, index: int, 
        ...
        self.id_entry.bind('<FocusOut>', lambda e: self.on_update_id(self.index, self.id_entry.get()))
...
class LessonEditorApp:
    ...
    def _create_ui(self):
        ...
        self.id_entry = ttk.Entry(row1, width=20)
    ...
    def _on_title_change(self, event=None):
        ...
        if not current_id or current_id.startswith('01_') or current_id == slugify(current_id.replace('01_', '')):
            new_id = f"01_{slugify(title)}"
    ...
    def _collect_all_data(self):
        ...
        self.lesson['id'] = self.id_entry.get()
        ...
        for i, card in enumerate(self.step_cards):
            self.lesson['steps'][i] = card.get_data()
    ...
    def load_file(self):
        ...
        try:
            ...
            # Update UI
            self.id_entry.delete(0, tk.END)
            self.id_entry.insert(0, self.lesson.get('id', ''))
