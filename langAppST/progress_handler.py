...
class ProgressStore:
    ...
    def lesson_completed(self, course_id: str, lesson_id: str, mistakes_made : int) -> None:
    ...
    def check_lesson_completed(self, course_id: str, lesson_id : str):
        if st.session_state["guest"]:
            return 0
        ...
        response = (
            ...
            .select("completed")
            .eq("user_id", self.supabase.auth.get_user().user.id)
    ...
    def get_completed_lessons(self, course_id: str):
        if st.session_state["guest"]:
            return []
        response = (
            self.supabase.table("lesson_progress")
    ...
    def get_recommended_lesson(self, course_id):
        if st.session_state["guest"]:
            return None
        response = (
            self.supabase.table("lesson_progress")
